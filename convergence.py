...
# convergence.py
"""
Manual convergence workflow

1) Run DPvsPPO.py for DP or ele_exp_actor.py for PPO multiple times with different EPISODES (e.g., 10, 100, 1000, 10000, ...).
2) Copy the printed DP or PPO summary lines into RAW_LINES below.
...
def parse_lines(raw_lines: List[str]) -> List[RunStat]:
    ...
    for line in raw_lines:
        m = LINE_RE.search(line)   # <-- search, not match
        if not m:
            raise ValueError(
                f"Could not parse this line:\n{line}\n\n"
                f"Expected fields like: mean=..., 95% CI=[..., ...], SD=..., N=..."
            )
...
# 4) Best-N selection
# =========================
def pick_best_n(dp_or_ppo_stats: List[RunStat]) -> RunStat:
    """
    ...
    """
    for s in dp_or_ppo_stats:
        if s.meets_tol:
            return s
    return dp_or_ppo_stats[-1]


# =========================
# 5) Plotting
# =========================
def plot_convergence(dp_or_ppo_stats: List[RunStat]):
    n = np.array([s.n for s in dp_or_ppo_stats], dtype=float)
    mean = np.array([s.mean for s in dp_or_ppo_stats], dtype=float)
    lo = np.array([s.ci_low for s in dp_or_ppo_stats], dtype=float)
...
def main():
    if not RAW_LINES:
        raise SystemExit(
            "RAW_LINES is empty.\n"
            "Run DPvsPPO.py or ele_exp_actor.py for different ELE_DP_N_EPISODES, copy the printed summary lines,\n"
            "paste them into RAW_LINES in convergence.py, then rerun convergence.py."
        )

    dp_or_ppo_stats = parse_lines(RAW_LINES)
    print("\nParsed runs:")
    for s in dp_or_ppo_stats:
    ...
    plot_convergence(dp_or_ppo_stats)
